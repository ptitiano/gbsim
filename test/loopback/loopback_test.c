	       ...
	       const char *postfix)
{
	...
	int request_avg, latency_avg, latency_gb_avg, throughput_avg;
	...
	len += snprintf(&buf[len], sizeof(buf) - len,
			"%s,%s,%s,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,%u",
		...
		for (i = 0; i < n; i++) {
			if (strstr(namelist[i]->d_name, "raw_latency_endo0")) {
				...
				snprintf(lb_name[j].dbgfs_entry, MAX_SYSFS_PATH,
					"%s%s", dbgfs_pfx, namelist[i]->d_name);
				if (debug)
